...
def handle(msg):
    def request_data():
        ...
        print("Searching for videos at: {}".format(url))
    ...
    if command.startswith('Ют') or command.startswith('yt') or comman.startswith('ют') or command.startswith('Yt'):
        data = request_data()
        ...
        filename = title+".mp3"
        bot.sendMessage(chat_id,"скачивает...")
